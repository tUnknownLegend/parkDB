...
-- CREATE INDEX IF NOT EXISTS userForum_idx_nickname_and_forum ON userForum (forum, nickname);

CREATE INDEX IF NOT EXISTS posts_idx_thread_and_path_and_parent ON posts (thread, (path[1]), parent, id NULLS FIRST);
CREATE INDEX IF NOT EXISTS posts_idx_thread_and_parent_and_id ON posts (thread, parent, id NULLS FIRST);
